...
class DbHandler{
	...
	private function _get_card_info($uid){
	...
	private function _checkTransaction($msisdn,$transid){
		$query = "SELECT id FROM card.transaction WHERE transid='$transid' and msisdn='$msisdn'";
		$row = $this->pdo_db->query($query)->fetch();
...
function p2p($transid,$utilityref,$msisdn,$amount){
		$proceed = "OK";
		$vendor = "SELCOMCARD";
		$service = "P2P";
